(() => {
  ...
  const STORAGE_KEY = "formularfuchs-tradein-v1";
  const DB_NAME = "formularfuchs-local";
  const STORE_NAME = "files";
  const TOTAL_STEPS = 7;
  const photoLabels = {
    overall: "Gesamtansicht des Geräts",
    serial: "Seriennummer / IMEI / Geräteinfo",
    displayOn: "Display / Gerät eingeschaltet",
    back: "Rückseite / Gehäuse",
  ...
  const defaultState = () => ({
    ...
    currentStep: 1,
    item: { name: "", manufacturer: "", model: "", serial: "", imei: "", reason: "" },
    ...
    offer: { originalAmount: "", originalClass: "", revisedAmount: "", revisedDate: "", revisedReason: "" },
  ...
  function accessoryList() {
    ...
    if (state.accessories.cable) items.push("Ladekabel / Anschlusskabel");
    if (state.accessories.manual) items.push("Originalunterlagen");
  ...
  function privacyList() {
    const items = [];
    if (state.privacy.backup) items.push("Backup erstellt");
    if (state.privacy.simRemoved) items.push("SIM entfernt");
    if (state.privacy.sdRemoved) items.push("SD-/Speicherkarte entfernt");
    if (state.privacy.accountRemoved) items.push("Konto entfernt");
    if (state.privacy.lockDisabled) items.push("Aktivierungssperre deaktiviert");
    if (state.privacy.reset) items.push("Werkseinstellungen");
    return items;
  }
  ...
  async function renderSummary() {
    ...
    html += '<div class="summary-card"><h3>Zustand, Angebot und Zubehör</h3>' +
      ...
      row("Ursprüngliches Angebot", euro(state.offer.originalAmount)) +
      row("Angegebene Zustandsklasse", state.offer.originalClass) +
      row("Mitgesendet", accessories.length ? accessories.join(", ") : "") +
      row("Account-/Daten-Check", privacyList().length ? privacyList().join(", ") : "") +
      '</div>';
